...
public class AITank2 : NetworkBehaviour
{
			...
			if(grounded && !quieto)
			{
				...
				}else if(Mathf.Abs((transform.position - target.position).x) >= distancia)
				{
					...
					/*if(Mathf.Abs(angle) < 15 && !seguir.GetComponent<vistaTank>().obstacle && !seguir.GetComponent<vistaTank>().obstacle)
					{
						...
						der = false;
						izq = false;
					}else */if (seguir.GetComponent<vistaTank>().obstacle || seguir.GetComponent<vistaTank>().obstacle)
					{
						adelante = false;
						//atras = true;
						if(lugar.x > 0.0f && !izq)
	...
	void Cmd_Disparo()
	{
		...
		granade.GetComponent<balaTankNetwork>().poder = saludMax*granade.GetComponent<balaTankNetwork>().poder/250;
	}
