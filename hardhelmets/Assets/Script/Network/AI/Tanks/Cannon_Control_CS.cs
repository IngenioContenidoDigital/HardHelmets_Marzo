...
*/
	...
	public void Update ()
	{
		if(objetivo != null )
		{
			...
			rot.x = transform.position.x;
			rot.z = transform.position.z;
			transform.LookAt(objetivo.transform);*/

			//transform.rotation = Quaternion.Euler(transform.localRotation.x,0,transform.rotation.z);

			otro = Quaternion.LookRotation(objetivo.transform.position - transform.position);
			otro.y = 0;//torreta.transform.rotation.y;
			otro.z = 0;//torreta.transform.rotation.z;
			transform.localRotation = Quaternion.Slerp(transform.localRotation, otro, 2 * Time.deltaTime);
		}else
		{
