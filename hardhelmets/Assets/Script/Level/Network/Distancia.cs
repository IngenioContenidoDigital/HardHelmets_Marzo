...
public class Distancia : NetworkBehaviour {
	...
	void Update ()
	{
		...
		if(Player1 != null && Objeto != null)
		{
			...
			{
				if(Objeto.GetComponent<HeroNetwork>())
				{
					if(Objeto.GetComponent<HeroNetwork>().salud > 0)
					{
						ver = true;
					}else
					{
	...
	void FixedUpdate ()
	{
		...
		if(Objeto == null)
		{
			Objeto = GameObject.Find(target2);
		}
		...
		if(Player1 != null && Objeto != null)
		{
			...
			float porcentaje = Objeto.transform.position.x / distancia;
			//iconoJugador.GetComponent<RectTransform>().localScale = Vector3.Lerp(minScale, maxScale, porcentaje);
			Vector3 taman = Vector3.Lerp(minScale, maxScale, porcentaje/division2);
			GetComponent<RectTransform>().localScale = taman;
